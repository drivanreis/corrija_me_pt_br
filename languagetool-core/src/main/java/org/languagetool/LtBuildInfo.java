...
public enum LtBuildInfo {
  ...
  LtBuildInfo(String gitPropertiesFilePath) {
    ...
    if (gitProperties != null) {
      ...
      if (buildTime != null) {
        OffsetDateTime offsetDateTime = OffsetDateTime.parse(buildTime, DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ssXX"));
      ...
      this.version = firstNonBlank(
        ...
        JLanguageTool.class.getPackage() != null ? JLanguageTool.class.getPackage().getImplementationVersion() : null,
        "dev"
      );
    } else {
      this.buildDate = null;
