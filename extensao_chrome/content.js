(() => {
  ...
  const statusNode = panel.querySelector("#cmpb-status");
  const resultsNode = panel.querySelector("#cmpb-results");
  ...
  function getExcerpt(match) {
    ...
    const end = Math.min(text.length, match.offset + match.length + 25);
    return text.slice(start, end).replace(/\s+/g, " ").trim();
  }

  async function analyzeActiveElement(manual) {
    ...
    try {
      ...
      setStatus(
        matches.length
          ? `${matches.length} sugestao${matches.length > 1 ? "es" : ""} encontrada${matches.length > 1 ? "s" : ""}.`
          : "Texto limpo. Nenhum ajuste necessario.",
        matches.length ? "" : "ok"
      );
      renderResults(matches);

      if (manual && !matches.length) {
        fab.textContent = "Sem erros";
        window.setTimeout(() => {
          if (activeElement) {
            fab.textContent = "Corrigir";
          }
      ...
      latestMatches = [];
      setStatus(
        `Nao foi possivel acessar o servidor local. Inicie o servidor em ${LOCAL_SERVER_URL}.`,
        "error"
      );
      renderResults([]);
    }
  ...
  function applySingleCorrection(index, replacement) {
    ...
    const updatedText = replaceTextRange(currentText, match.offset, match.length, replacement);
    setText(activeElement, updatedText);
    latestText = updatedText;
    analyzeActiveElement(false);
  }
  ...
  function applyAllCorrections() {
    ...
    let text = getText(activeElement);
    const applicableMatches = latestMatches
      .filter((match) => Array.isArray(match.replacements) && match.replacements.length)
      .sort((left, right) => right.offset - left.offset);

    if (!applicableMatches.length) {
      setStatus("Nao ha correcoes automaticas para aplicar.", "");
      return;
    }

    for (const match of applicableMatches) {
      text = replaceTextRange(text, match.offset, match.length, match.replacements[0].value);
    }
  ...
  document.addEventListener("focusout", () => {
    window.setTimeout(() => {
      ...
      if (!isSupportedElement(next) && !panel.contains(next)) {
        hideFab();
      }
    }, 60);
  });
  ...
  document.addEventListener("keydown", (event) => {
    if (event.altKey && event.shiftKey && event.key.toLowerCase() === "c") {
      ...
      analyzeActiveElement(true);
    }
  });

  fab.addEventListener("click", () => analyzeActiveElement(true));
  panel.querySelector("[data-close]").addEventListener("click", hidePanel);
  panel.querySelector("[data-refresh]").addEventListener("click", () => analyzeActiveElement(true));
