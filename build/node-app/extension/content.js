...
(() => {
  ...
  function normalizeCompositeToken(text) {
    return text.normalize("NFC").trim().toLocaleLowerCase("pt-BR");
  }
  function buildSpecialCompositeGroups(sourceText, targetText) {
    const sourceTokens = tokenizeWithOffsets(sourceText);
    const targetTokens = tokenizeWithOffsets(targetText);
    if (sourceTokens.length < 2 || targetTokens.length < 3 || targetTokens[1]?.text !== "-") {
      return null;
    }
  ...
  function mergeAdjacentDiffGroups(groups, sourceText) {
    if (groups.length < 2) {
      return groups;
    }
    const merged = [];
  ...
  function expandCompositeMatches(matches, text) {
    ...
    matches.forEach((match) => {
      ...
      const validGroups = mergeAdjacentDiffGroups(
        buildSpecialCompositeGroups(sourceText, primaryReplacement) || buildTokenDiffGroups(tokenizeWithOffsets(sourceText), tokenizeWithOffsets(primaryReplacement)),
        sourceText
      ).slice(0, 6);
