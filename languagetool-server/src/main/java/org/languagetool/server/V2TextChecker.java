...
class V2TextChecker extends TextChecker {
  ...
  protected void checkParams(Map<String, String> parameters) {
    ...
    if (!StringTools.isEmpty(language)) {
      parsePortugueseBrazilLanguage(language);
    }
    ...
    if (parameters.get("autodetect") != null) {
      throw new BadRequestException("Automatic language detection is not supported by this server. Use Brazilian Portuguese ('pt-BR').");
    }
  }
  ...
  protected DetectedLanguage getLanguage(String text, Map<String, String> parameters, List<String> preferredVariants,
    ...
    Language language = parsePortugueseBrazilLanguage(parameters.get("language"));
